...
# use LaTeX in text
plt.rcParams.update({'text.usetex': True, 'font.family': 'sans-serif', 'font.sans-serif': 'Arial', 'font.size': 12})
...
def plot(dim):
    files = np.loadtxt("../geometries_vmr_richter22.txt", dtype="str")
    ...
    for j, fname in enumerate(files):
        ...
        # with open(os.path.join(FDIR, "vmr", fname + ".json")) as f:
        with open(os.path.join(FDIR, "vmr", "geometric", fname + "_0d.in")) as f:
            inp = json.load(f)
        # with open(os.path.join(FDIR, "vmr", fname + "_optimal" + end)) as f:
        with open(os.path.join(FDIR, "vmr", "calibrated_from_" + str(dim) + "d", fname + "_0d.in")) as f:
            opt = json.load(f)
    ...
    if dim == 0:
        xtext = "0D elements in forward simulation (ground truth)"
        ytext = "0D elements in inverse calibration"
    elif dim == 3:
        xtext = "Geometric 0D elements"
        ytext = "Calibrated 0D elements from 3D results"
